...
class KGModel(nn.Module, ABC):
    ...
    def __init__(self, args, edge_index, edge_type, sizes, rank, dropout, gamma, data_type, bias, init_size):
        """Initialize KGModel."""
        ...
        self.xvaier = args.xvaier
        self.gamma = nn.Parameter(torch.Tensor([gamma]), requires_grad=False)

        self.entity = nn.Embedding(sizes[0], rank, sparse=args.sparse)
        self.rel = nn.Embedding(sizes[1], rank, sparse=args.sparse)
        
            
        if data_type == 'double':
            self.entity.weight.data = self.entity.weight.data.double()
            self.rel.weight.data = self.rel.weight.data.double()
        
        if self.xvaier:
            nn.init.xavier_uniform_(tensor=self.entity.weight)
        else:
            self.entity.weight.data *= self.init_size 
        ...
        self.gcn = GCNLayer(edge_index, edge_type, self.data_type, self.rank, self.rank, self.sizes[1], self.sizes[0], args, act=torch.tanh, head_num=args.num_head)
        if args.double_layer:
            self.gcn2 = GCNLayer(edge_index, edge_type, self.data_type, self.rank, self.rank, self.sizes[1], self.sizes[0], args, act=torch.tanh, head_num=args.num_head)
    ...
    def get_factors(self, queries, entity, rel_diag):
    ...
    def forward(self, queries, eval_mode=False):
        ...
        """
        if self.no_gcn:
            entity = self.entity.weight
        else:
            entity = self.gcn(self.entity.weight, self.c)
            if self.double_layer:
                entity = self.gcn(entity, self.c)

        # get embeddings and similarity scores
        lhs_e, lhs_biases = self.get_queries(queries, entity)
        # queries = F.dropout(queries, self.dropout, training=self.training)
        rhs_e, rhs_biases = self.get_rhs(queries, entity, eval_mode)
        ...
        factors = self.get_factors(queries, entity)
        return predictions, factors

    def get_ranking(self, queries, filters,  entity, batch_size=1000):
        ...
        with torch.no_grad():
            b_begin = 0
            candidates = self.get_rhs(queries, entity, eval_mode=True)
            while b_begin < len(queries):
                these_queries = queries[b_begin:b_begin + batch_size].cuda()

                q = self.get_queries(these_queries,  entity,)
                rhs = self.get_rhs(these_queries, entity, eval_mode=False)
    ...
    def compute_metrics(self, examples, filters, batch_size=500):
        ...
        for m in ["rhs", "lhs"]:
            ...
            if m == "lhs":
                ...
                q[:, 2] = tmp
                q[:, 1] += self.sizes[1] // 2
            ranks = self.get_ranking(q, filters[m],  entity, batch_size=batch_size)
