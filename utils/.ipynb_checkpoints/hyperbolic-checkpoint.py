...
def dot_multi_c(x, v, c, eval_mode=False):
    ''' 
       x: euclidean embedding (B x d)
       v: euclidean embedding (B x d)  else (N x d)
       c: curvature (B x 1)
    '''
    sqrt_c = c ** 0.5
    xnorm = torch.norm(x, p=2, dim=-1, keepdim=True)   # (Bx1)
    
    if eval_mode:
        vnorm = torch.norm(v, p=2, dim=-1, keepdim=True).transpose(0, 1) # (1x N)
        cos_g = (x / xnorm.clamp_min(MIN_NORM)) @ (v.transpose(0, 1) / vnorm.clamp_min(MIN_NORM))                             # (B x N)
    else:
        vnorm = torch.norm(v, p=2, dim=-1, keepdim=True)   # (Bx1)
        cos_g = torch.sum((x/ xnorm.clamp_min(MIN_NORM)) * (v / vnorm.clamp_min(MIN_NORM)) , dim=-1, keepdim=True)    # (Bx1)
    
    gamma_x = tanh(sqrt_c * xnorm) / sqrt_c     #(B x 1)
    gamma_v = tanh(sqrt_c * vnorm) / sqrt_c     # (1xN)
    
    num = gamma_x * gamma_v * cos_g        # (B x N)
    denom = (1 + (c * gamma_x * xnorm) **2) * (1 + (c * gamma_v * vnorm) **2) - c**2 * num
    return num / denom


def dot_multi_c_2(x, v, c, eval_mode=False):
    ''' 
       x: euclidean embedding (B x d)
       v: euclidean embedding (B x d)  else (N x d)
       c: curvature (B x 1)
    ...
    sin_g = torch.sqrt(1 - cos_g**2)         # （B x 1） or (B x N)
    gamma_x = tanh(sqrt_c * xnorm) / sqrt_c     # (B x 1)
    x_M = gamma_x / (1 - sqrt_c ** 2 * gamma_x ** 2)      #(B x 1)
    h_M = x_M * sin_g    #(B x 1) or (B x N)
    gamma_h = 2 * h_M / (1 + torch.sqrt(1 + 4 * c * h_M ** 2))  #(B x 1) or (B x N)
    x2v = torch.sqrt(torch.abs(gamma_x ** 2 - gamma_h **2 )/(1 - c**2 * gamma_x ** 2 * gamma_h **2) + 1e-6)
    gamma_v = tanh(sqrt_c * vnorm) / sqrt_c     # (1xN) or (Bx1)
